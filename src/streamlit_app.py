...
def call_deepseek_api(prompt, max_tokens=300, temperature=0.7):
    ...
    # 专业的八字命理师系统提示
    system_prompt = """你是一个中国传统八字命理的专业研究人员。
    你熟读《穷通宝鉴》、《三命通会》、《滴天髓》、《渊海子平》、《千里命稿》、《协纪辨方书》、《果老星宗》、《子平真诠》、《神峰通考》等一系列经典命理书籍。
    你精通八字排盘、日主分析、十神配置、大运流年、五行生克等命理技术。
    请基于专业的八字命理知识，结合用户的个人信息，给出准确、专业、实用的命理分析。
    回答时要体现专业深度，引用经典命理著作的理论，同时要温暖贴心，让用户容易理解。"""
...
def get_wuxing_strength(day_master):
    """分析五行属性"""
    wuxing_map = {
        "甲": "阳木", "乙": "阴木", "丙": "阳火", "丁": "阴火",
        "戊": "阳土", "己": "阴土", "庚": "阳金", "辛": "阴金", 
...
def init_session_state():
    ...
    if "bazi_info" not in st.session_state:  # 新增八字信息
...
def render_daily_fortune():
    ...
    if st.session_state.daily_fortune is None:
        with st.spinner("🔮 正在占卜今日运势..."):
            st.session_state.daily_fortune = generate_daily_fortune(zodiac, birth_info, bazi_info)
...
def render_personal_recommendation():
    ...
    user_question = st.text_input("输入您的问题...", key="chat_input_personal")
    if st.button("发送", key="send_personal") and user_question.strip():
        st.session_state.chat_history.append({"role": "user", "content": user_question})
